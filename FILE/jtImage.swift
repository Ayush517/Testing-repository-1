...
public struct jtImage {
    ...
    public init(jpeg url: URL, imageFormat: pixelFormats = .RGB888, channelCount: Int32 = 3) {
        ...
        guard let loadedData = try! LoadJPEG(atPath: url.path, imageFormat: finalPixel) else {
            // TODO: Proper error propagation for this.
    ...
    public func save(to url: URL, numberOfChannels: Int = 3, quality: Int64 = 95) {
        ...
        return outputImageData.scalars.withUnsafeBufferPointer { bytes in
            ...
            let status = try! SaveJPEG(atPath: url.path, image: self.imageData, dataPointer: tensorPointer)
            guard status == 0 else {
                // TODO: Proper error propagation for this.
