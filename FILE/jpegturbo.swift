...
func SaveJPEG(atPath path: String, image: ImageData, dataPointer: UnsafeMutablePointer<UInt8>) throws -> Int32 {
    do {
    ...
    /* Compress the Image Data from `image.data` into `jpegBuf`
     ...
     - `flags` = 0
     */
    tjCompress2(compressor, dataPointer, image.width, 0, image.height, image.formatProperties.rawValue, &jpegBuf, &jpegSize, 0, outQual, 0)
